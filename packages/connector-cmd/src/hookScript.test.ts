 ...
 * that matter:
 *
 * - no `POSEIDON_HOOK_URL` and no bearer — an interactive `cmd` run in a project
 ...
 * - env configured but the bridge unreachable — an Poseidon session whose
 ...
 *   hook's env, and a live run proved it strips `POSEIDON_HOOK_TOKEN`.
 */
...
const runHook = (
  ...
  try {
    ...
    delete env.POSEIDON_HOOK_URL;
    delete env.POSEIDON_HOOK_TOKEN;
    delete env.POSEIDON_HOOK_TICKET_FILE;
...
describe("cmd-hook.mjs under real node", () => {
  it("emits NO decision when the Poseidon env is absent", () => {
  ...
  it("emits NO decision when only the token is missing", () => {
    const result = runHook({ POSEIDON_HOOK_URL: "http://127.0.0.1:9/hooks/pretooluse" });
    expect(result.status).toBe(0);
    expect(result.stdout).toBe("");
  });
  ...
  it("still denies when the env is set but the bridge is unreachable", () => {
    const result = runHook({
      ...
      POSEIDON_HOOK_URL: "http://127.0.0.1:9/hooks/pretooluse",
      POSEIDON_HOOK_TOKEN: "t",
    });
    expect(result.status).toBe(0);
   ...
   * `POSEIDON_HOOK_URL` and `POSEIDON_THREAD_ID` reaching a hook while
   * `POSEIDON_HOOK_TOKEN` did not — the CLI redacts TOKEN, BEARER, SECRET, AUTH,
