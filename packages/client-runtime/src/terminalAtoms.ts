 ...
 * run when it is set again, which would drop keystrokes typed while an earlier
 * one was in flight. Input goes through one lane per terminal instead
 * (`makeInputLanes`), so it reaches the shell in the order it was typed however
 ...
 * join. That is fine for the three calls nobody reads the answer of, but the
 * drawer reads the summary `openTerminal` answers with, so that one is a
 * one-shot: two tabs opening together each get their own terminal.
 *
  ...
  const openTerminal = (
    ...
    runOneShot(runtime, registry, () =>
      Effect.gen(function* () {
        ...
        Effect.ensuring(Effect.sync(() => registry.refresh(terminalListAtom(args.threadId)))),
      ),
    );
