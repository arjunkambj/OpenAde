...
and `{pause: ms}` is the recorder waiting between two commands.
...
| `cli-reap`                  | a live daemon in the namespace: `session info` (pid, socket dir), `session list`, `close --all`, the list emptied |
