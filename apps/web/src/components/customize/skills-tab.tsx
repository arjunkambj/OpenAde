/**
 * The Skills tab: the listing `cmdConfig.skills.list` reports for the chosen
 * scope — user skills under the connector's user skills root, project skills
 * under the project's, project winning name collisions. A skill's content
 * lives in its file, and the path line is the way to it.
 *
 * Below it, the skills in the shared agents folder (`~/.agents/skills`) that
 * the connector does not load yet. Adding one links it into the global skills
 * root; nothing is copied, so the agents folder stays the source.
 */
...
export function SkillsTab() {
  ...
  const agentSkillsResult = useAtomValue(atoms.agentSkillsAtom);
  ...
  const addSkill = async (entry: string) => {
    setLinking(entry);
    const exit = await link({ entry, projectId });
    setLinking(null);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Hidden when there is nothing to add: an empty agents folder is the
          common case, not a state worth a placeholder. */}
      {agentSkills.length === 0 ? null : (
        <CustomizeSection title="In ~/.agents/skills" count={agentSkills.length}>
          {agentShown.length === 0 ? (
            <CustomizeEmpty>No skills match “{query.trim()}”.</CustomizeEmpty>
          ) : (
